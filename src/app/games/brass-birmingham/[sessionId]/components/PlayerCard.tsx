...
export function PlayerCardFull({
  ...
  const btnBase =
    "h-10 border border-border text-sm font-medium active:bg-muted disabled:opacity-25";
    ...
    <div className="overflow-hidden rounded-xl shadow-sm">
      ...
      <div className="bg-card">
        ...
        <Row label={t.playerCard.money} value={ps.money}>
          <button onClick={() => adjustMoney(-1)} disabled={moneyEmpty} className={`${btnBase} w-12 rounded-l-lg bg-muted/60`}>−</button>
          <button onClick={() => adjustMoney(1)} className={`${btnBase} w-12 border-l-0 bg-muted/80`}>+</button>
          <button onClick={takeLoan} disabled={loanBlocked} className={`${btnBase} w-14 rounded-r-lg border-l-0 bg-muted/60 text-xs`}>{t.playerCard.loan}</button>
        </Row>

        {/* Spent */}
        <Row label={t.playerCard.spent} value={spent} border>
          <button onClick={() => adjustSpend(spent - 1)} disabled={spent <= 0} className={`${btnBase} w-12 rounded-l-lg bg-muted/60`}>−</button>
          <button onClick={() => recordSpend(1)} disabled={moneyEmpty} className={`${btnBase} w-12 border-l-0 bg-muted/80`}>+</button>
          <button onClick={() => recordSpend(5)} disabled={moneyEmpty} className={`${btnBase} w-14 rounded-r-lg border-l-0 bg-muted/60 text-xs`}>+5</button>
        </Row>
        ...
        <Row
          ...
          value={
            <>
              ...
              <span className={payout >= 0 ? "text-emerald-600" : "text-red-600"}>
                £{payout}
              </span>
            </>
          }
        >
          <button onClick={() => adjustIncome(-1)} disabled={ps.income <= 0} className={`${btnBase} w-12 rounded-l-lg bg-muted/60`}>−</button>
          <button onClick={() => adjustIncome(1)} disabled={ps.income >= 99} className={`${btnBase} w-12 rounded-r-lg border-l-0 bg-muted/80`}>+</button>
        </Row>
        ...
        <Row
          ...
          border
          value={<span className="font-bold text-amber-600">{ps.vp}</span>}
        >
          <button onClick={() => adjustVP(-1)} disabled={ps.vp <= 0} className={`${btnBase} w-12 rounded-l-lg bg-muted/60`}>−</button>
          <button onClick={() => adjustVP(1)} className={`${btnBase} w-12 border-l-0 bg-muted/80`}>+</button>
          <button onClick={() => adjustVP(5)} className={`${btnBase} w-12 border-l-0 bg-muted/60 text-xs`}>+5</button>
          <button onClick={() => adjustVP(10)} className={`${btnBase} w-12 rounded-r-lg border-l-0 bg-muted/60 text-xs`}>+10</button>
        </Row>
      </div>
    </div>
  );
...
export function PlayerCardCompact({ player, gameState }: PlayerCardCompactProps) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-card shadow-xs px-4 py-2.5">
      ...
      <div className="flex items-center gap-3 text-xs tabular-nums text-muted-foreground">
        ...
        <span className={payout >= 0 ? "text-emerald-600/80" : "text-red-600/80"}>
          {payout >= 0 ? `+£${payout}` : `-£${Math.abs(payout)}`}
        </span>
        {ps.vp > 0 && <span className="text-amber-600/80">{ps.vp}VP</span>}
      </div>
    </div>
  );
}
...
function Row({
  ...
  return (
    <div className={`flex items-center justify-between px-4 py-2.5 ${border ? "border-t border-border/40" : ""}`}>
