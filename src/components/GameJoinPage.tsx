...
function JoinContent({ gameSlug }: { gameSlug: string }) {
  ...
  useEffect(() => {
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (msg.type === "CODE_FOUND") {
        socket.close();
        router.replace(
          `/games/${gameSlug}/${msg.roomId}?action=join&name=${encodeURIComponent(name)}`
        );
...
export function GameJoinPage({ gameSlug }: GameJoinPageProps) {
  return (
